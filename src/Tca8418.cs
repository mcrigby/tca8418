...
public sealed class Tca8418 : IDisposable
{
    private readonly I2cDevice _i2CDevice;
    private readonly Tca8418Configuration _tca8418Configuration;
    private readonly GpioPin _interruptPin;
    private readonly GpioPin _resetPin;

    public Tca8418(I2cDevice i2CDevice, GpioController gpioController, Tca8418Configuration tca8418Configuration)
    {
        _i2CDevice = i2CDevice;
        _tca8418Configuration = tca8418Configuration;

        _interruptPin = gpioController.OpenPin(_tca8418Configuration.InterruptPinNumber, System.Device.Gpio.PinMode.InputPullDown);
        _interruptPin.ValueChanged += InterruptOccurred;

        _resetPin = gpioController.OpenPin(_tca8418Configuration.ResetPinNumber, System.Device.Gpio.PinMode.Output, PinValue.High);
    ...
    public bool Matrix(byte rows, byte columns) 
    {
        ...
        //  MATRIX
        //  reset zero size matrix
        if ((rows == 0) || (columns == 0)) 
        {
        // Set to GPIO instead of KP
        _i2CDevice.WriteRegister(Tca8418Register.KP_GPIO_1, 0x00);
        _i2CDevice.WriteRegister(Tca8418Register.KP_GPIO_2, 0x00);
        _i2CDevice.WriteRegister(Tca8418Register.KP_GPIO_3, 0x00);
        return true;
        }
    ...
    public void InterruptOccurred(object? sender, PinValueChangedEventArgs eventArgs)
    {
        if (eventArgs.ChangeType == PinEventTypes.Rising)
            return;

        byte intStat = _i2CDevice.ReadRegister(Tca8418Register.INT_STAT);
        bool keyEvent = (intStat & 0x01) == 0x01;
        bool gpioEvent = (intStat & 0x02) == 0x02;
    ...
    {
        _i2CDevice.WriteRegister(Tca8418Register.INT_STAT, 0x1F);
    }
    ...
    public static Tca8418 Create(GpioController gpioController, Tca8418Configuration tca8418Configuration, byte busId = DefaultBusId, byte deviceAddress = DefaultDeviceAddress)
    {
        ...
        return new(i2CDevice, gpioController, tca8418Configuration);
    }

    public const byte DefaultBusId = 1;
    public const byte DefaultDeviceAddress = 0x34;
